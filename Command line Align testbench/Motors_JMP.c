...
#define REFRESH_RATE 20972      //Refresh ratio
#define DEADBAND 1573           // DeadBand ratio
#define RANGE 420               // Range for PWM
#define DEADBAND_CORRECTION 32          //Positive DeadBand offset
#define NTP_RATIO 2          //Negative to positive ratio
#define PWM_TO_OPERATIONS 1.05  //Convert PWM to operations

#define P1 -0.001
#define P2 3
#define P3 2163

#define IDEAL_RANGE 400
#define IDEAL_DEADBAND 1500
...
void MOTOR_speed(int speed, unsigned int motor){
    if((speed < MOTOR_CAP + 1) && (speed > -1*(MOTOR_CAP+1) ) && (motor < 9)){          // Verify correct parameters
        unsigned long operations = (speed*RANGE)/100 + DEADBAND;        // Calculate appropriate duty cycle
        if(speed > 0){
            operations += DEADBAND_CORRECTION;
        }else if(speed < 0){
            operations *= PWM_TO_OPERATIONS*(P1*(speed*IDEAL_RANGE*speed*IDEAL_RANGE/10000)+ P2*speed + P3));
           //operations *= NTP_RATIO;
            operations += DEADBAND_CORRECTION;
        }
