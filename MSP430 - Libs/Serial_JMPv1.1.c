...
void UART_receive_byte(char *dest){
    UARTBlock = 1;							  // Wait for user input
...
/*\**********************************************************************************************************
 *Function Description
 * 	Receives one Byte of information from the UART serial line without waiting for input
 *Parameters
 *	Dest - destination were the byte will be stored
 *\**********************************************************************************************************/

void UART_receive_non_blocking(){
    UARTBlock = 0;							  // Dont block until user input
    UCA0IE |= UCRXIE;                         // Enable USCI_A0 RX interrupt
}
...
/*\**********************************************************************************************************
 *Function Description
 * 	Receives one Byte of information from the UART serial line wihout blocking
 *Parameters
 *	Dest - destination were the byte will be stored
 *\**********************************************************************************************************/
...
void USB_receive_line(char *dest){
    ...
    LPM0;									 // Go into low power mode to wait for a byte
    *dest = USBBuff;						 // Assign the first value to the array
    while (*dest){							 // While we dont receive a null pointer
        LPM0;								 // Wait for new variable
        *++dest = USBBuff;				     // Increment pointer and assign new value
    }
    UCA1IE &= ~UCRXIE;                       // Disable USCI_A0 RX interrupt
}
...
void USB_print_float(char *string, float value){
    char buffer[16];
    int integer = value;
    float midpoint = (float)(value - integer);
    int fractional = (int)(midpoint*1000);
    itoa(value, buffer);
    unsigned char *intArr = (unsigned char *)buffer;
    while(*string){
        USB_transmit_byte(*string);
    ...
    USB_transmit_byte('.');
    itoa(fractional, buffer);
    intArr = (unsigned char *)buffer;
    while(*intArr){
        USB_transmit_byte(*intArr);
        intArr++;
    }
