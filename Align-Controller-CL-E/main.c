...
void forwards(int time, int speed){
    ...
    while(!timeout){
        ...
        //MOTOR_speed(speed, R_MOTOR);
        //MOTOR_speed(speed, L_MOTOR);
        MOTOR_speed(speed, H_MOTORS);
...
int main(void) {
    ...
    //char cmd[COMMAND_BUFFER_SIZE], *transactionComplete = 0;	 //Old snippet of code
    cmd = malloc(BUFFER_SIZE);
    transactionComplete = malloc(sizeof(char));
